<template>
  <ValidationObserver v-slot="{ handleSubmit }">
    <form class="space-y-6" @submit.prevent="handleSubmit(addToBranch)">
        ...
        <div class="px-4 py-5 bg-white sm:p-6">
          <div class="grid grid-cols-6 gap-6">
            <div class="col-span-6 sm:col-span-4">
              ...
              >{{ editableBranch._id ?"Edit" : "Add" }} Branch </label>
            </div>
            <div class="col-span-6 sm:col-span-3">
              <label for="title" class="ml-0 block text-sm font-medium text-black-700">Branch name</label>
...
export default {
  ...
  methods: {
    setEditableData() {
      if (this.editableBranch._id) {
        let data = JSON.parse(JSON.stringify(this.editableBranch));
        delete data._id;
        this.branch = data;
      } else {
        this.branch = {
    ...
    closeForm() {
      this.$modal.hide("branch-form");
    },
    async addToBranch() {
      this.isLoading = true;
      if (this.editableBranch._id) {
        await axios.put(
          "/branches/" + this.editableBranch._id,
          this.branch
        );
      } else {
        await axios.post("/branches", this.branch);
      }
      this.branch = {
        title: ""
      };
