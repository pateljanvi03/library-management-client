<template>
  <ValidationObserver v-slot="{ handleSubmit }">
    <form class="space-y-6" @submit.prevent="handleSubmit(issueBook)">
      <div class="shadow sm:rounded-md border border-gray-300">
        <div class="px-4 py-5 bg-white sm:p-6">
          <div class="grid gap-6">
            ...
            <div class="grid grid-cols-2 place-content-between sm:col-span-3">
              <ValidationProvider name="Enrollment" rules="required" v-slot="{ errors }">
                <input
                  v-model="enrollmentNo"
              ...
              <button type="button" @click="findStudent" class="place-items-end mt-2 ml-4 w-20 h-10 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">Search</button>
              <div v-if="!isEmpty" class="mt-4">
                <p v-if="student[0] == undefined" class="text-red-600">Student not found</p>
                <div v-else >
                <p>Name: {{student[0].name}}</p>
                <p>Branch: {{student[0].branch.title}}</p>
...
export default {
  ...
  methods: {
    ...
    async issueBook() {
      this.isLoading = true;
      this.$emit("studentId", this.student[0]._id);
      this.$emit("refresh");
    ...
    async findStudent() {
        const response = await axios.get('/students', {
            params: {enrollmentNumber: this.enrollmentNo}
        });
