<template>
  <ValidationObserver v-slot="{ handleSubmit }">
    <form class="space-y-6" @submit.prevent="handleSubmit(addBookItem)">
        ...
        <div class="px-4 py-5 bg-white sm:p-6">
          <div class="grid grid-cols-6 gap-6">
            <div class="col-span-6 sm:col-span-4">
              ...
              >{{ editableBookItem ?"Edit" : "Add" }} BookItem </label>
            </div>

            <div class="col-span-6 sm:col-span-3">
              <label for="bookId" class="ml-0 block text-sm font-medium text-black-700">Book</label>
              <ValidationProvider name="Book" rules="required" v-slot="{ errors }">
                <select
                  v-model="bookItem.bookId"
            ...
            <div class="col-span-6 sm:col-span-4">
              <label for="sequence" class="block text-sm font-medium text-black-700">Shelf</label>
              <ValidationProvider name="Shelf" rules="required" v-slot="{ errors }">
                <input
                  v-model="bookItem.shelf"
                  type="string"
                  name="shelf"
...
export default {
  ...
  methods: {
    setEditableData() {
      if (this.editableBookItem._id) {
        let data = JSON.parse(JSON.stringify(this.editableBookItem));
        delete data._id;
        this.bookItem = data;
      } else {
        this.bookItem = {
    ...
    async addBookItem() {
      this.isLoading = true;
      if (this.editableBookItem._id) {
        await axios.put("/bookItems/" + this.editableBookItem._id, this.bookItem);
      } else {
        await axios.post("/bookItems", this.bookItem);
      }
